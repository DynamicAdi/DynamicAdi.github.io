...
function Sidepane({opened, setOpen, color, setColor, tabs}: {
  opened: string,
    ...
    {
      tabs.map((tab, index) => (
        <div key={index} onClick={() => {setOpen(tab.name), setColor(tab.color)}} className={`relative lg:hover:ml-4 cursor-pointer ${opened === tab.name && `bg-${color}/20 text-${color}` } ${isDarkMode ? "text-white": "text-black"} transition-all ease-in-out duration-300 lg:py-1 lg:px-4 p-3 rounded-lg lg:w-4/5 w-1/5 lg:text-left text-center lg:text-2xl`}><h3 className={`text-2xl flex lg:justify-start justify-center items-center lg:gap-2 `}><tab.icon className={`lg:text-2xl text-3xl`}/><span className='hidden lg:inline'>{tab.name}</span></h3></div>
      ))
    }
   </div>
    </div>
